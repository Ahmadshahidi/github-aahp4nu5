/*
  ...
  3. Helper Functions
    - `user_has_resource_access()` - Check if user can access a specific resource
    - `user_has_global_access()` - Check if user has active global access
    - `get_user_accessible_resources()` - Get all resources user can access
    - `purchase_resource()` - Purchase individual resource access
    - `purchase_global_access()` - Purchase all-access pass
...
-- Create user_resource_access table
...
-- Create global_access_purchases table
...
-- Helper function: Check if user has active global access
...
-- Helper function: Purchase individual resource access
CREATE OR REPLACE FUNCTION public.purchase_resource(resource_uuid uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF NOT resource_record.is_paid THEN
    RETURN json_build_object('success', false, 'error', 'Resource is not a paid resource');
  END IF;
  
  -- Check if user already has access
  IF public.user_has_resource_access(resource_uuid) THEN
    RETURN json_build_object('success', false, 'error', 'User already has access to this resource');
  END IF;
  
  -- Insert access record
  INSERT INTO public.user_resource_access (user_id, resource_id)
  VALUES (auth.uid(), resource_uuid)
  ON CONFLICT (user_id, resource_id) DO NOTHING;
  
  RETURN json_build_object(
    'success', true, 
    'message', 'Resource access purchased successfully',
    'resource_id', resource_uuid,
    'price', resource_record.price
  );
END;
$$;

-- Helper function: Purchase global access
CREATE OR REPLACE FUNCTION public.purchase_global_access(duration_months integer DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Check if user already has active global access
  IF public.user_has_global_access() THEN
    RETURN json_build_object('success', false, 'error', 'User already has active global access');
  END IF;
  ...
  -- Deactivate any existing global access
  UPDATE public.global_access_purchases
  SET is_active = false
  WHERE user_id = auth.uid() AND is_active = true;
  
  -- Insert new global access record
  INSERT INTO public.global_access_purchases (user_id, valid_until)
  VALUES (auth.uid(), expiry_date);
  
  RETURN json_build_object(
    'success', true,
    'message', 'Global access purchased successfully',
