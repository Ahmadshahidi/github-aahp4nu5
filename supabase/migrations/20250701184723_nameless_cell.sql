...
-- Ensure courses table has proper structure by adding missing columns
DO $$
BEGIN
...
-- Create course_purchases table to track individual course payments if it doesn't exist
...
DROP POLICY IF EXISTS "Users can view their own course purchases" ON public.course_purchases;
DROP POLICY IF EXISTS "Users can create their own course purchases" ON public.course_purchases;
DROP POLICY IF EXISTS "Service role can manage all course purchases" ON public.course_purchases;
DROP POLICY IF EXISTS "Users can access courses they have purchased or with subscription" ON public.courses;
